...
// 1. Whitelist for live TSDB matches
const MAJOR = {
...
// 2. Fallback league names when ESPN omits them
...
// 4. Smart JSON fetch (only TSDB v2 needs the header)
...
// 7. Core: pull up to 5 matches per sport
async function getMatches(sport) {
  ...
  // 2️⃣ ESPN fallback: upcoming & completed
  if (results.length<5) {
...
// 8. Four endpoints
app.get("/scores/soccer",            async (req,res)=>{ const m=await getMatches("soccer");            res.json(m.length?m:[{headline:"No major soccer games right now."}]); });
app.get("/scores/nba",               async (req,res)=>{ const m=await getMatches("basketball");        res.json(m.length?m:[{headline:"No NBA games right now."}]); });
app.get("/scores/nfl",               async (req,res)=>{ const m=await getMatches("american_football"); res.json(m.length?m:[{headline:"No NFL games right now."}]); });
app.get("/scores/nhl",               async (req,res)=>{ const m=await getMatches("ice_hockey");        res.json(m.length?m:[{headline:"No NHL games right now."}]); });
