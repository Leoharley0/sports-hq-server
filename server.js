...
// Set these in Render → Environment (no angle brackets)
const V2_KEY = process.env.TSDB_V2_KEY || process.env.TSDB_KEY || "342128";
const V1_KEY = process.env.TSDB_V1_KEY || "1"; // public test key works for v1 upcoming
...
// NEVER use strTime as a status (it caused false LIVE)
...
// v2 livescore with header; try both routes and sport spellings
...
async function getLeagueMatches(sport, leagueId) {
  ...
  // 2) UPCOMING (fill to 5) – v1 endpoint uses URL key
  if (out.length < 5) {
