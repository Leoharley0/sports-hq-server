// server.js — resilient TSDB fetch (retry/backoff), better NFL fill, verbose logs.
const express = require("express");
const app = express();
const PORT = process.env.PORT || 3000;

const BUILD = "SportsHQ v7 (retry+backoff, nfl fill)";
...
if (!V2_KEY) console.error("❌ Missing TSDB_V2_KEY (v2 livescore)");
if (!V1_KEY) console.warn("⚠️ Missing TSDB_V1_KEY (v1 season/day/next/past)");
  ...
  for (let i = 0; i < tries; i++) {
    try {
      ...
      if (res.status === 429) {
        ...
        console.warn(`[retry] 429 on ${url} -> wait ${wait}ms (try ${i+1}/${tries})`);
        await sleep(wait);
        continue;
      }
      ...
      console.warn(`[retry] error on ${url} -> ${e.message}; wait ${wait}ms (try ${i+1}/${tries})`);
      await sleep(wait);
    }
  }
  console.error(`[fetchJsonRetry] failed after ${tries} tries -> ${lastErr?.message} :: ${url}`);
  return null;
}
...
// -------------- day-fill until n --------------
async function fillByDaysUntil(out, sport, leagueId, n, maxWeeks){
...
async function getLeagueMatchesCore(sport, leagueId, n){
  ...
  // 4) DAY scan — NFL needs deeper fill (use 10 weeks), others 6
  const weeks = (sport==="american_football") ? 10 : 6;
  await fillByDaysUntil(out, sport, leagueId, n, weeks);
