...
const BUILD = "SportsHQ v24 (resilient seasons for EPL & NFL; no day-scan)";
...
// Robust candidates per sport:
function seasonCandidatesSoccer(){ // cross-year first, then single-year
  const c=guessSeasonCrossYear(), y=(new Date()).getUTCFullYear();
  return [c, nextCross(c), prevCross(c), String(y), String(y+1), String(y-1)];
}
function seasonCandidatesNFL(){ // single-year first, then cross-year (both formats)
...
async function buildSingle({ sport, leagueId, n, seasons }){
  ...
  const finalsRaw=(past||[]).filter(isRecentFinal);
  finalsRaw.sort((a,b)=>eventMillis(b)-eventMillis(a));
  ...
  const nextFut=(next||[]).filter(e=>{const ms=eventMillis(e); return !Number.isNaN(ms) && isFinite(ms) && ms>=now;})
  ...
  if (finals.length+live.length+sched.length < n){
    for (const s of seasons){
      ...
      // keep future rows; if time is missing but date exists, toMillis gives midnight → ok
      const fut=(rows||[]).filter(e=>{
        const ms=eventMillis(e);
        if (Number.isNaN(ms)) return false; // no date at all, skip
...
app.get("/scores/soccer", async (req,res)=>{
  try{
    ...
    const items=await getBoardCached(`OUT:soccer_epl_v24:${n}`, () =>
...
async function serveSingle(req,res,sport,leagueId,seasons){
  try{
    const n=Math.max(5, Math.min(10, parseInt(req.query.n||DEFAULT_COUNT,10)||DEFAULT_COUNT));
    const items=await getBoardCached(`OUT:${sport}:${leagueId}:v24:${n}`, () =>
