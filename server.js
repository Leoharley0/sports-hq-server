...
const BUILD = "SportsHQ v23 (unified single-league builder; no day-scan)";
...
async function buildSingle({ sport, leagueId, seasons, n }){
  ...
  // Live (v2, filtered by league)
  const liveAll = await v2Livescore(sport);
  for (const m of (liveAll||[])) if (!isRecentFinal(m) && String(m.idLeague||"")===String(leagueId)) pushUnique(live, m);

  // Next league (cheap; usually ~15 soonest)
  const next = (await v1NextLeague(leagueId))
    .filter(e=>{ const ms=eventMillis(e); return isFinite(ms) && ms>=now; })
  ...
  // Seasons (current → next → prev) – only if still short
...
async function getBoardCached(key, builder){
  ...
  const fetcher=async()=>{
    ...
    setCache(key, data, full?TTL.OUT:8_000); // short cache for partial lists
    return data;
  };
  if (c.hit && !c.fresh){ withInflight(key, fetcher); return c.val; }
  return withInflight(key, fetcher);
...
// ===== routes (ALL use the same unified builder) =====
app.get("/scores/soccer", async (req,res)=>{
  try{
    const n = Math.max(5, Math.min(10, parseInt(req.query.n || DEFAULT_COUNT,10) || DEFAULT_COUNT));
    const items = await getBoardCached(`OUT:soccer_epl_v23:${n}`, () =>
      buildSingle({ sport:"soccer", leagueId:L.EPL, seasons:seasonsCrossTriple(), n })
    );
...
async function serveSingle(req,res,sport,leagueId,seasons){
  try{
    const n = Math.max(5, Math.min(10, parseInt(req.query.n || DEFAULT_COUNT,10) || DEFAULT_COUNT));
    const items = await getBoardCached(`OUT:${sport}:${leagueId}:v23:${n}`, () =>
